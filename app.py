    ...
    if st.button("✨ Filter starten"):
        if not artikel_liste:
            ...
            if filtered.empty:
                ...
                target_url = st.text_input("Link eingeben (Webhook / API / Upload-Ziel):")

                if st.button("📤 An Link senden"):
                    if not target_url:
                        st.error("Bitte zuerst einen Link eingeben!")
                    else:
                        files = {"file": ("gefilterte_artikel.xlsx", output.getvalue())}
                        try:
